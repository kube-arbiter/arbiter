...
func (ex *Arbiter) Score(ctx context.Context, state *framework.CycleState, pod *v1.Pod, nodeName string) (score int64, newState *framework.Status) {
	...
	workqueue.ParallelizeUntil(ctx, DefaultParallelism, len(scoreResults), func(piece int) {
	...
	}, workqueue.WithChunkSize(chunkSizeFor(len(scoreResults), DefaultParallelism)))
	msg := strings.Builder{}
...
// copy from https://github.com/kubernetes/kubernetes/blob/v1.23.10/pkg/scheduler/framework/parallelize/parallelism.go
